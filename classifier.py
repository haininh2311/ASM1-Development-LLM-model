...
class LlamaEmbeddingClassifier(torch.nn.Module):
	...
	def forward(self, input_ids):
		...
		# Apply dropout (self.dropout) to the hidden state at training time to mitigate overfitting.
		last_hidden_state = self.dropout(last_hidden_state)

		# Pass this through the classifier head (self.classifier_head), which will return logits (unnormalized probabilities) over all classes.
		logits = self.classifier_head(last_hidden_state)

		# Take the log-softmax of the logits and return log-probabilities over all classes.
		log_probabilities = F.log_softmax(logits, dim=-1)
